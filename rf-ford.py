...
df_baru = pd.DataFrame([data_mobil_baru])
print("\n--- Memprediksi Harga Mobil Baru ---")
print("Data mobil baru:")
print(df_baru)

df_baru_encoded = pd.get_dummies(df_baru, columns=['model', 'transmission', 'fuelType'])
df_baru_final = df_baru_encoded.reindex(columns=train_columns, fill_value=0)

prediksi_harga = rf_model.predict(df_baru_final)

print(f"\nPrediksi Harga Mobil: ${prediksi_harga[0]:,.2f}")
